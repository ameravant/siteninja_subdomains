...
protected
  def create_unique_permalink
    ...
    # if self.class.respond_to?(:account_id) && self.class.permalink_options[:scope] = :account_id
    #       conditions.first << " and #{self.class.permalink_options[:scope]} = ?"
    #       conditions       << send(self.class.permalink_options[:scope])
    #     end
    #     unless self.class.respond_to?(:account_id)
    #       while self.class.exists?(conditions)
    #         suffix = "-#{counter += 1}"
    #         conditions[1] = "#{base[0..limit-suffix.size-1]}"
...
if Object.const_defined?(:Iconv)
  ...
  PermalinkFu.translation_to   = 'ascii//ignore//translit'  #modified to work with Windows-based systems
  # PermalinkFu.translation_from = 'utf-8' #removed to work with Windows-based systems
end